...
public class LabirintoBuilderTest {
	...
	@Test
	public void testLabirintoConStanzaBuia_SenzaAttrezzoIlluminante() {

	}

	@Test
	public void testLabirintoConStanzaBuia_ConAttrezzoIlluminante() {
	...
	@Test
	public void testLabirintoConMagoEAttrezzoDaRegalare() {
		String nomeMago = "Merlino";
		String nomeAttrezzo = "spada";
		int pesoAttrezzo = 1;
		String messaggio = "ciao";
		Labirinto lab = this.labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaVincente(nomeStanzaVincente)
				.addMago(nomeMago, messaggio, nomeAttrezzo, pesoAttrezzo)
				.getLabirinto();
		Mago mago = (Mago)lab.stanzaVincente.getPersonaggio();
		//Verifichiamo che il mago � nell'ultima stanza aggiunta al labirinto
		assertNull(lab.getStanzaIniziale().getPersonaggio());
		assertNotNull(mago);
		//Verifichiamo che i parametri del mago sono corretti
		assertEquals(mago.getNome(),nomeMago);
		assertEquals(mago.getPresentazione(),messaggio);
		//Verifichiamo che l'attrezzo da regalare � quello corretto
		assertEquals(mago.getAttrezzoDaRegalare(),new Attrezzo(nomeAttrezzo,pesoAttrezzo));
		//Verifichiamo che il tipo di personaggio � quello corretto
		assertEquals(mago.getClass(),Mago.class);
	}

	@Test
	public void testLabirintoConStrega() {
		String nomeStrega = "Merlina";
		String messaggio = "ciao";
		Labirinto lab = this.labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaVincente(nomeStanzaVincente)
				.addStrega(nomeStrega, messaggio)
				.getLabirinto();
		//Verifichiamo che la strega sia stata messa nell'ultima stanza aggiunta al labirinto
		assertNull(lab.getStanzaIniziale().getPersonaggio());
		assertNotNull(lab.getStanzaVincente().getPersonaggio());
		//Verifichiamo che i parametri della strega siano corretti
		AbstractPersonaggio strega = lab.getStanzaVincente().getPersonaggio();
		assertEquals(strega.getNome(),nomeStrega);
		assertEquals(strega.getPresentazione(),messaggio);
		//Verifichiamo che la strega e' effettivamente un oggetto di tipo strega
		assertEquals(strega.getClass(),Strega.class);
	}

	@Test
	public void testLabirintoConCane() {
		String nomeCane = "Fido";
		String messaggio = "woof";
		String nomeCiboAppetitoso = "Bistecca";
		int peso = 99;
		Labirinto lab = this.labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addStanzaVincente(nomeStanzaVincente)
				.addCane(nomeCane, messaggio, "Attrezzo Regalato", peso, nomeCiboAppetitoso)
				.getLabirinto();
		//Verifichiamo che la strega sia stata messa nell'ultima stanza aggiunta al labirinto
		assertNull(lab.getStanzaIniziale().getPersonaggio());
		assertNotNull(lab.getStanzaVincente().getPersonaggio());
		//Verifichiamo che i parametri della strega siano corretti
		Cane cane = (Cane)lab.getStanzaVincente().getPersonaggio();
		assertEquals(cane.getNome(),nomeCane);
		assertEquals(cane.getPresentazione(),messaggio);
		assertEquals(cane.getNomeCiboAppetitoso(),nomeCiboAppetitoso);
		assertEquals(cane.getAttrezzoDaRegalare(),new Attrezzo("Attrezzo Regalato",peso));
		//Verifichiamo che la strega e' effettivamente un oggetto di tipo strega
		assertEquals(cane.getClass(),Cane.class);
	}
}
